...
The compact adult-content prompt is added only when the mode is enabled and the current turn clearly concerns adult content. Clear local matches require no classifier call. Other direct bot messages in an enabled group are classified by `deepseek_v4_flash`, with the configured official provider used after primary failures or invalid output. New slang or euphemisms do not require source updates. Undirected group traffic and commands are not classified. Repeated identical messages use a bounded hash cache. A short continuation can inherit the prompt from the newest conversation turns. Ordinary messages receive no additional Gemini prompt tokens.
