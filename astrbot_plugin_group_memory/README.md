...
- Each stored native turn names its speaker, and bot replies retain their target member.
...
- Relevant short-term history and a bounded set of long-term memories are injected as temporary LLM context. Recent records are not duplicated when identity-bound native history is already available.
...
Short-term records are retained for 14 days with at most 160 records per group by default. Per-request retrieval is limited to four relevant records and 2,500 characters, while long-term recall is limited to 900 characters. The roster includes at most 12 recent members. Long-term memories use a 180-day half-life, a 730-day hard limit, recall reinforcement with a 12-hour cooldown, and a 300-memory group cap. Permission checks use the shared hierarchy; memory limits remain configurable in AstrBot WebUI.
